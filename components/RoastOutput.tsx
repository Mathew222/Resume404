...
import { Trash2, MessageSquare, Skull, RefreshCw, Volume2, Square, Tv, TrendingUp, Gem, Hammer, CheckCircle, Download, FileCode, FileText, Copy, X } from 'lucide-react';
...
const RoastOutput: React.FC<RoastOutputProps> = ({ roast, originalContent, loadingState, onReset }) => {
  ...
  const handleFixResume = async () => {
    setIsFixing(true);
    try {
      const contentToFix = originalContent || roast; // Fallback to roast if original empty (shouldn't happen with new logic)
      const data = await fixResume(contentToFix);
      setFixedData(data);

      // Auto download PDF
      // generateAndDownloadPDF(data); // Optional: maybe let user choose to download? kept for now.
  ...
  const generateAndDownloadPDF = (data: FixedResumeData) => {
    ...
    // Header (Name roughly)
    const headerLines = doc.splitTextToSize(data.pdfContent.header, 180);
    doc.text(headerLines, 105, y, { align: 'center' });
    ...
    // Sections
    data.pdfContent.sections.forEach((section) => {
      if (y > 270) { doc.addPage(); y = 20; }

      doc.setFont("times", "bold");
      doc.setFontSize(14);
      doc.text(section.title.toUpperCase(), 10, y);
      y += 6;

      doc.setFont("times", "normal");
      doc.setFontSize(11);
      const bodyLines = doc.splitTextToSize(section.body, 190);
      doc.text(bodyLines, 10, y);

      y += (bodyLines.length * lineHeight) + 5;
    });

    doc.save("Fixed_Resume_ATS_Optimized.pdf");
  };
  ...
  const copyToClipboard = () => {
    if (!fixedData) return;
    navigator.clipboard.writeText(fixedData.latex);
    setCopySuccess(true);
  ...
  return (
    <div className="w-full animate-fade-in-up pb-20">
              ...
              <div className="flex flex-col gap-3 w-full md:w-auto shrink-0">
                {!fixedData ? (
                  ...
                  <div className="flex flex-col gap-2 animate-fade-in-up">
                    ...
                    >
                      <FileCode size={16} /> Preview LaTeX Code
                    </button>
                  </div>
                )}
      ...
      {/* PREVIEW MODAL */}
      {showPreview && fixedData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-paper border-4 border-ink w-full max-w-4xl max-h-[90vh] flex flex-col shadow-hard-lg">
            <div className="flex justify-between items-center bg-ink text-white p-4">
              <h3 className="font-display font-black uppercase text-xl flex items-center gap-2">
                <FileCode size={20} className="text-yellow-400" />
                LaTeX Source Code
              </h3>
              <button onClick={() => setShowPreview(false)} className="hover:text-alert transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-0 flex-grow relative overflow-hidden">
              <textarea
                readOnly
                value={fixedData.latex}
                className="w-full h-[60vh] bg-[#1e1e1e] text-gray-300 font-mono text-sm p-4 resize-none focus:outline-none"
              />
              <button
                onClick={copyToClipboard}
                className="absolute top-4 right-4 bg-yellow-400 text-black font-bold uppercase text-xs px-3 py-1 flex items-center gap-2 border-2 border-transparent hover:bg-white hover:border-yellow-400 transition-colors shadow-sm"
              >
            ...
            <div className="p-4 bg-gray-100 border-t-4 border-ink flex justify-end gap-3">
              <button
                onClick={() => setShowPreview(false)}
                className="px-4 py-2 font-bold uppercase text-gray-600 hover:text-ink hover:underline"
              >
                Close
              </button>
              <button
                onClick={downloadLatex}
                className="px-6 py-2 bg-ink text-white font-bold uppercase hover:bg-gray-800 transition-colors flex items-center gap-2"
              >
